...
def show_individual_lesion_explorer(visualizer, tasks_df):
    ...
    if selected_lesion:
        ...
        with col4:
            if pd.notna(lesion_info.get('Clinical score', np.nan)) and pd.notna(lesion_info.get('Outcome score', np.nan)):
                improvement = lesion_info['Clinical score'] - lesion_info['Outcome score']
        ...
        try:
            ...
            if len(treatment_volume_df) > 0:
                treatment_volume_df['Improvement'] = (
                    treatment_volume_df['Clinical score'] - treatment_volume_df['Outcome score']
                )
                ...
                fig_volume_response = px.scatter(
                    ...
                    labels={'volume': 'Lesion Volume (voxels)', 'Improvement': 'Improvement Score'}
                )
        ...
        with st.spinner("Generating comprehensive analysis report..."):
            try:
                ...
                with col1:
                    ...
                    st.write(f"**Patients with Deficits:** {summary['patients_with_deficits']}")
                    st.write(f"**Treatment Patients:** {summary['treatment_patients']}")
                
                with col2:
                    st.write(f"**Control Patients:** {summary['control_patients']}")
                    st.write(f"**Mean Clinical Score:** {summary['mean_clinical_score']:.2f} ± {summary['std_clinical_score']:.2f}")
                ...
                if 'error' not in report['treatment_efficacy']:
                    ...
                    st.write(f"**Treatment Group:** {efficacy['treatment_mean_improvement']:.2f} mean improvement")
                    st.write(f"**Control Group:** {efficacy['control_mean_improvement']:.2f} mean improvement")
                    st.write(f"**Statistical Significance:** p = {efficacy['ttest_p_value']:.4f}")
                ...
                if 'error' not in report['response_analysis']:
                    ...
                    st.write(f"**Overall Response Rate:** {response['response_rate']:.2%}")
                    st.write(f"**Number of Responders:** {response['n_responders']}")
                    st.write(f"**Number of Non-responders:** {response['n_non_responders']}")
                ...
                if 'error' not in report['volume_analysis']:
                    ...
                    st.write(f"**Mean Lesion Volume:** {volume['mean_volume']:.0f} ± {volume['std_volume']:.0f} voxels")
                    st.write(f"**Volume-Clinical Correlation:** {volume['volume_clinical_correlation']:.3f}")
