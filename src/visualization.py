...
class BrainLesionVisualizer:
    ...
    def plot_lesion_slices(
        self, lesion_id: str, figsize: Tuple[int, int] = (15, 5)
    ) -> plt.Figure:
        """
        ...
        if len(coords[0]) > 0:
            ...
            center_x, center_y, center_z = (
                lesion_data.shape[0] // 2,
                lesion_data.shape[1] // 2,
                lesion_data.shape[2] // 2,
            )
    ...
    def plot_clinical_score_distribution(self) -> go.Figure:
        ...
        fig.add_trace(
            go.Histogram(
                x=treatment_scores, name="Treatment group", opacity=0.7, nbinsx=20
            )
        )
    ...
    def get_lesion_volumes_summary(self) -> pd.DataFrame:
        ...
        volume_df = pd.DataFrame({"lesion_id": lesion_ids, "volume": volumes})
...
def visualize_random_lesions(
    visualizer: BrainLesionVisualizer, n_samples: int = 6
) -> plt.Figure:
    """
