...
X_task1 = X_train_all[task1_mask]  # type: ignore
y_task1 = train_df.loc[task1_mask, "clinical_score"].values  # type: ignore

model_task1 = ElasticNetCV(
    cv=5, random_state=42, l1_ratio=[0.1, 0.5, 0.7, 0.9, 0.95, 0.99, 1]
)
model_task1.fit(X_task1, y_task1)

joblib.dump(model_task1, MODELS_DIR / "task1_atlas_model.pkl")
print(
    f"Task 1 model saved. Best L1 ratio: {model_task1.l1_ratio_:.2f}, Alpha: {model_task1.alpha_:.4f}"
)
...
X_task2 = X_train_all[task2_mask]  # type: ignore
y_task2 = train_df.loc[task2_mask, "is_responder"].values  # type: ignore
